...
**项目状态：** M5 中英混输与键盘交互收敛预览；等待 GitHub Actions 与 Android 真机验收

**最新预览：** `v0.3.2-m5`（`versionCode 8`，prerelease）
...
## 0. 当前迭代：v0.3.2-m5 中英混输与键盘交互收敛

`v0.3.2-m5` 继续停留在 `0.3.x`：本轮让候选、剪贴板、Emoji、文字编辑和中英混输更接近可日常使用状态，但 GitHub Actions 与 Android 真机质量门禁尚未完成。Android `versionCode` 从 7 单调增加到 8；发布 tag 固定为 `v0.3.2-m5`，只允许创建 prerelease。

本轮保留 `ygz → 一个字`、`w → 我`、`pipei → 匹pei → 匹配`、候选展开、高速触控、加速退格和九宫格等已有能力，并加入：

- composition 和候选批次分 revision 发布，pending 时不再清空候选造成闪烁；新批次只在完整就绪后原子替换；
- 输入时由候选占用品牌工具栏空间，竖屏建议键盘高度增至 400 dp、横屏 300 dp；
- 剪贴板每行一个全宽卡片，预览按 Unicode code point 裁剪为最多两行且不会越过卡片；
- Emoji 使用连续纵向滚动，滚动成立后取消该 pointer 的元素点击；
...
当前验证状态如下。只有 GitHub Actions 的 `verify` 作业全部通过，工作流才会创建 `v0.3.2-m5` prerelease；未实际执行的 Kotlin、Lint、APK 或真机检查不会写成“已通过”。
...
| Android 真机 | 尚未连接；候选稳定帧、连续 Emoji 滚动、编辑页、OEM 事件合并、120 Hz 帧时间和横竖屏待测 |
...
本次发布仍是工程预览：Host 状态机与延迟门禁不能替代真机质量结论。固定发布签名尚未建立，每次 GitHub runner 生成的 debug 证书可能不同，因此即使 `versionCode` 增加，`v0.3.2-m5` 也不保证能覆盖安装旧 APK。签名不一致时卸载旧版会同时清除本地 SQLite 用户词库和剪贴板历史。完整决策见 [`ADR 0007`](docs/adr/0007-v0.3.2-m5-bilingual-input-and-keyboard-panels.md)，变更与限制见 [`v0.3.2-m5 发布说明`](docs/releases/v0.3.2-m5.md)。
...
| M6 事件底座 | Journal、Segment、Blob、Manifest、100 万事件回放 | 高负载采集不影响输入；崩溃恢复无逻辑丢段 |
| M7 AI 垂直切片 | `:brain`、AIDL、OpenAI-compatible、流式、6 个 Skill | Brain 被杀不影响输入；异步结果不覆盖新文本 |
| M8 记忆与快照 | 词汇/纠错/App/风格记忆、索引、快照编译、A/B 回放 | 个性化相对基线出现可重复增益 |
| M9 稳定化 | 机型矩阵、24h 压测、安装包、故障降级、发布检查 | 所有 P0 门禁通过，形成首个内测版本 |

连续多轮真实输入反馈使 M3–M5 优先关闭候选质量、渐进选词、高速触控、工具面板和中英混输缺口。事件事实底座顺延到 M6，AI、记忆和稳定化阶段依次顺延；四方向 Skill、完整事件归档和 Arctic Glass 三级回退仍保留在后续里程碑。

## 12. 迭代记录：M0 可运行骨架

以下保留 M0 的实施记录；当前代码位于 M5 的 `v0.3.2-m5` 工程预览，仍需完成 GitHub Actions、Android 真机安装、SQLite/剪贴板进程恢复、中英混输质量、连续滚动、文字选择与高速输入性能验收。
...
**下一步：** 先让 `v0.3.2-m5` 的 GitHub Actions 阻断门禁全部通过，再在 Android 10+ 真机完成启用、切换、`host / fun / zhongwsrf / zhongwensrf / pipei`、候选稳定帧、剪贴板裁剪、Emoji 连续滚动、文字选择、高速触控、横竖屏与候选延迟 Macrobenchmark。固定预览签名建立前，不把旧 APK 覆盖安装视为已支持能力。
