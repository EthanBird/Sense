...
**项目状态：** M2 自适应中文输入预览已构建；等待 Android 真机学习闭环验收
...
M2 开始解决“能输入”到“好输入”的差距：Rime pinyin-simp 的带频候选与低权重 CC-CEDICT 补充层合并后形成 97,536 个 exact 拼音键；构建器导入 80,777 个此前缺失的 `(拼音, 词语)` 对，经每码最多 128 个候选的上限截断后，最终保留 80,525 个。隔离的 1～4 字母统计短码使 `w` 的首选稳定为“我”，不会污染 `xian` 等真实全拼键。有界移动键盘编辑搜索支持最长 48 字母内的一次换位、漏键、多键或邻键误触。SQLite 用户词库持久化实际上屏短语，同时在内存索引全拼和简拼；完整输入 `woshiyigeren → 我是一个人` 一次后，下一次输入 `wsygr` 即可召回。候选按实际宽度从左侧依次排列，不再六等分居中。APK 不声明网络权限。

| 本地门禁 | 结果 |
|---|---|
| 词库构建器测试 | 3/3 通过 |
| `core-input` 单元测试 | 27/27 通过 |
| `ime-ui` 布局契约测试 | 5/5 通过 |
...
以下保留 M0 的实施记录；当前代码已推进到 M2，仍需完成 Android 真机安装、SQLite 重启恢复与输入性能验收。
