...
**项目状态：** M8-A AI 编辑协议与 Fake Harness 开发预览；完整保留 M7 输入能力

**最新预览：** `v0.3.5-m8-a`（`versionCode 11`，prerelease）
...
## 0. 当前迭代：v0.3.5-m8-a AI 编辑协议与 Fake Harness

`v0.3.5-m8-a` 继续停留在 `0.3.x`，是 AI 接入的第一道基础门禁。它先冻结编辑器快照、结构化 Patch、流式事件、会话状态和代际取消协议，不接入真实网络、不读取宿主输入框，也不让模型输出直接上屏。Android `versionCode` 从 10 单调增加到 11；发布 tag 固定为 `v0.3.5-m8-a`，只允许创建 prerelease。

本轮完整保留 M7 的固定键盘高度、扩充词库、英文组合态、横滑候选、Emoji 惯性、22 类符号、语义候选、文字编辑、高速触控、加速退格和九宫格，并加入：

- 独立纯 Kotlin/JVM `ai-protocol` 模块，不依赖 Android、HTTP 或 Provider SDK；
- 版本化 `EditorSnapshotV1`、`HarnessRequestV1` 与 `EditorPatchV1`；
- 严格 JSON 解析、未知字段/重复键/非法 Unicode/越界正文拒绝；
- 有界 Harness 状态机、Fake 流式事件、唯一终结结果和 generation 取消门禁；
- 普通输入和 APK 仍不声明 `INTERNET`，M8-A 不进入逐键热路径。

当前验证状态如下。只有 GitHub Actions 的 `verify` 作业全部通过，工作流才会创建 `v0.3.5-m8-a` prerelease；未实际执行的 Kotlin、Lint、APK 或真机检查不会写成“已通过”。

| 门禁 | 当前状态 |
|---|---|
| M8-A 协议 | 严格 Patch parser/validator、快照能力、请求绑定和 Unicode/大小边界测试 |
| M8-A Harness | Fake stream、取消、超时、迟到事件、重复终结和非法状态转移测试 |
...
| APK 元数据门禁 | `versionCode 11`、`versionName 0.3.5-m8-a`、`minSdk 29`、`targetSdk 36` |
| APK 完整性门禁 | zipalign、签名、三项模型哈希、20,000 英文词、内置许可、无 `INTERNET` 权限 |
| Android 真机 | M8-A 无新增运行时入口；M7 输入与 OEM 兼容矩阵继续待真机验证 |
...
本次发布仍是工程预览：M8-A 只建立可验证的协议和 Fake Harness，不宣称长按空格、Provider 或真实 AI 已可用。固定发布签名尚未建立，每次 GitHub runner 生成的 debug 证书可能不同，因此即使 `versionCode` 增加也不保证覆盖安装旧 APK。签名不一致时卸载旧版会同时清除本地 SQLite 用户词库和剪贴板历史。完整决策见 [`ADR 0010`](docs/adr/0010-v0.3.5-m8-ai-editor-harness.md)，变更与限制见 [`v0.3.5-m8-a 发布说明`](docs/releases/v0.3.5-m8-a.md)。
...
| M8 AI 编辑 Harness | A 协议/Fake Harness；B Provider 配置与 Brain 边界；C 长按空格与固定高度 AI Surface；D 快照与原子 Patch；E 真实 Provider | 松手取消 0 次误覆盖；旧快照 0 次上屏；Brain 故障不影响普通输入 |
...
连续多轮真实输入反馈使 M3–M7 优先关闭候选质量、渐进选词、高速触控、工具面板、中英混输、字符可达性、键盘几何和默认词库缺口。M8 改为优先交付用户明确要求的 AI 编辑 Harness，并以协议、Provider 边界、交互、编辑事务、真实模型五个可发布子阶段推进；完整事件归档、长期记忆和四方向 Skill 顺延，但仍保留模块边界。

## 12. 迭代记录：M0 可运行骨架

以下保留 M0 的实施记录；当前代码位于 M8-A 的 `v0.3.5-m8-a` 协议工程预览。M8-A 不提供真实 AI 入口；现有输入仍需继续完成 Android 真机安装、SQLite/剪贴板进程恢复、空 composing 跨宿主兼容、候选与 Emoji 惯性、符号字体和高速输入性能验收。
...
**下一步：** 先让 `v0.3.5-m8-a` 的严格协议、Fake Harness、既有输入回归、Lint、APK 和无网络权限门禁全部通过；随后进入 M8-B Provider Profile、Keystore 与私有 `:brain` Fake Service。长按空格、读取输入框和真实 Provider 分别留在 M8-C、M8-D、M8-E，不能越过前一阶段的取消与覆盖安全门禁。
