...
**项目状态：** M4 渐进式中文输入与高速键盘工程预览；等待 Android 真机输入与性能验收
...
## 0. 当前迭代：M4 渐进式中文输入与高速键盘

M4 优先关闭真实输入反馈中的可用性和性能缺口。词典新增 32,372 个精确简拼索引键，`ygz` 可直接得到“一个字”，`w` 的首候选仍为“我”；拼音状态支持合法音节边界的渐进式选择，例如 `pipei → 匹pei → 匹配`。回车原样确认当前混合组合，空格选择剩余拼音的第一候选，完整上屏继续自动写入本地 SQLite 用户词库。

触控改为按 pointerId 独立冻结目标并完整处理多点事件，2 ms 交叠按键不设防抖；候选解码迁出 IME 主线程，由单 worker、单 pending 和 revision 校验保证最新结果一致。退格支持连续加速，字母支持向上滑字符，数字页改为九宫格，剪贴板与 Emoji 页面补全，功能键统一使用 Canvas 图标。普通输入仍完全本地运行，APK 不声明网络权限。
...
| APK 词典许可 | NOTICE、CC-CEDICT 说明、CC BY-SA 4.0 正文均已内置 |
| 网络权限 | 未声明 |
...
本次发布仍是工程预览：当前环境没有连接 Android 设备，因此安装、启用、真实 InputConnection、多点高速触控、横竖屏和帧时间 Macrobenchmark 仍标为待测，Host 状态机与延迟门禁不能替代真机质量结论。M3 发布 APK 使用临时 debug 证书，M3 到 M4 可能无法覆盖安装；卸载旧版会清除其本地 SQLite 用户词库和剪贴板历史。成熟 librime 内核仍按后续阶段评估接入。
...
| M4 渐进式输入与高速键盘 | 简拼索引、自动分词、异步解码、多指触控、加速退格、完整工具页 | Host/CI 门禁通过；等待真机高速输入和视觉验收 |
...
以下保留 M0 的实施记录；当前代码已推进到 M4，仍需完成 Android 真机安装、SQLite/剪贴板重启恢复、渐进式候选质量与高速输入性能验收。
...
**下一步：** 在 Android 10+ 真机安装 `v0.4.0-m4`，完成启用、切换、`ygz / pipei` 渐进输入、每秒十余键高速触控、长按退格、九宫格、剪贴板、Emoji、横竖屏与候选延迟 Macrobenchmark；用真实回放继续扩充词库和纠错样本。
