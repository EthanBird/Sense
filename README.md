...
**目标预览：** `v0.3.5-m8`（`versionCode 12`，prerelease）
...
## 0. 当前迭代：v0.3.5-m8 AI 编辑

已发布的协议预览冻结了编辑器快照、结构化 Patch、流式事件与代际取消协议。其余 Provider、私有 Brain、长按空格、AI Surface、编辑器事务和真实网络统一在一次 `v0.3.5-m8` 集成迭代中完成、测试、编译和发布，不再创建中间阶段。Android `versionCode` 从 11 单调增加到 12，tag 固定为 `v0.3.5-m8`。
...
只有 GitHub Actions 的 `verify` 作业全部通过，工作流才会创建 `v0.3.5-m8` prerelease；未实际执行的 Kotlin、Lint、APK 或真机检查不会写成“已通过”。
...
| APK 元数据门禁 | `versionCode 12`、`versionName 0.3.5-m8`、`minSdk 29`、`targetSdk 36` |
...
以下保留 M0 的实施记录；当前代码位于 `v0.3.5-m8` AI 编辑集成预览。现有输入仍需继续完成 Android 真机安装、SQLite/剪贴板进程恢复、空 composing 跨宿主兼容、候选与 Emoji 惯性、符号字体和高速输入性能验收。
...
**下一步：** 让 `v0.3.5-m8` 的协议、Provider、编辑器、普通输入、Lint、APK、权限与发布门禁在同一提交中通过；随后在 Pixel、Samsung、小米/Redmi、OPPO/vivo 上验收长按/松手竞态、自定义编辑器兼容、Brain 进程故障和真实 Provider。
