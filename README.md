...
**项目状态：** M4 候选、手势与学习闭环修复预览；等待 GitHub Actions 与 Android 真机验收

**最新预览：** `v0.3.1-m4`（`versionCode 7`，prerelease）
...
## 0. 当前迭代：v0.3.1-m4 候选、手势与学习闭环修复

`v0.3.1-m4` 针对第三轮真实输入反馈收紧版本声明和发布门禁。版本名回到 `0.3.x`，因为候选可达性、触控手势、一次选择学习与真机质量尚未满足稳定版标准；Android `versionCode` 仍由 6 单调增加到 7。历史 `v0.4.0-m4` 文档和标签保留，但不再代表当前成熟度判断。

本轮保留 `ygz → 一个字`、`w → 我`、`pipei → 匹pei → 匹配`、异步解码、多点高速输入、加速退格、九宫格、剪贴板和无边框 Emoji 等已有能力，并集中修复以下阻断问题：

- 上滑字符使用冻结的起始键和整段轨迹判定；阈值为 `max(12dp, 键高 × 18%)`，短促斜向上滑不再要求抬手仍落在原键窄区域；
- 解码最多保留 255 个整词和 255 个渐进前缀候选，服务层以 510 项有界快照交给 UI；首屏先保留 12 个整词，再放分词候选，剩余整词仍可从展开面板翻页访问；
- 精确词条不再终止合法自动分词，生产回放新增 `hua → 滑`（前 64 项可达）和 `shanghua → 上滑`（前 255 项可达）；
- 展开候选支持上一页、下一页和纵向翻页手势，所有页继续使用稳定的全局候选索引；折叠态只测量可见候选，展开后再缓存全量宽度；
- Emoji 网格使用 Android touch slop 锁定纵向翻页；手势成立后永久取消本次元素点击，避免“滑动变点击”；
- Shift、退格、回车、键盘和剪贴板图标统一按当前按键短边计算比例、线宽和安全边界；
- pending 与 decoded 两态使用同一合成标题锚点、字号和基线；完整 `PinyinComposition` 与 revision 共同隔离异步结果，旧候选不能回填；
- 动态前缀保留可验证的规范全拼，显式选择一次先同步更新内存排序，再异步写入 SQLite。最近选择优先于旧高频记录，`d → 的` 下一次立即置顶并可由持久记录恢复。

当前验证状态如下。只有 GitHub Actions 的 `verify` 作业全部通过，工作流才会创建 `v0.3.1-m4` prerelease；本地缺少可用的 Gradle 8.13/Android 构建工具链，因此没有把未执行的 Kotlin、Lint 或 APK 检查写成“本地通过”。

| 门禁 | 当前状态 |
|---|---|
| 拼音词库、Bigram 与生产资产 Python 测试 | 11/11 本地通过 |
| 纯 Kotlin 回归测试 | 独立编译并执行 `core-input` 54 项、`ime-service` 13 项、`ime-ui` 34 项，共 101/101 本地通过；正式 Gradle/JUnit 仍由 GitHub Actions 阻断验证 |
| M4 正确性回放 | 新增 `hua/滑`、`shanghua/上滑`、`d/的/de`，并保留 `w`、`ygz`、`pipei`；由 GitHub Actions 执行 |
| M4 Host 性能门禁 | 简拼 ≤ 250 µs；16 项渐进解析 ≤ 500 µs；255 项渐进解析与分词组合各 ≤ 5 ms |
| Android Lint 与编译 | GitHub Actions 构建 Debug、Benchmark 与 Macrobenchmark APK；尚未在本地执行 |
| APK 元数据门禁 | `versionCode 7`、`versionName 0.3.1-m4`、`minSdk 29`、`targetSdk 36` |
| APK 完整性门禁 | zipalign、v2 签名、词典/Bigram 哈希、内置许可、无 `INTERNET` 权限 |
| Android 真机 | 尚未连接；InputConnection、OEM 事件合并、120 Hz 帧时间、横竖屏和进程恢复待测 |
...
本次发布仍是工程预览：当前环境既没有可用的本地 Gradle 发行版，也没有连接 Android 设备，最终构建结论以 GitHub Actions 为准；Host 状态机与延迟门禁不能替代真机质量结论。固定发布签名仍未建立，每次 GitHub runner 生成的 debug 证书可能不同，因此即使 `versionCode` 增加，`v0.3.1-m4` 也不保证能覆盖安装任何旧 APK。签名不一致时卸载旧版会同时清除本地 SQLite 用户词库和剪贴板历史。成熟 librime 内核仍按后续阶段评估接入。完整变更与限制见 [`docs/releases/v0.3.1-m4.md`](docs/releases/v0.3.1-m4.md)。
...
| M4 渐进式输入与高速键盘 | 简拼索引、自动分词、异步解码、多指触控、加速退格、完整工具页 | `v0.3.1-m4` 新门禁通过后，等待真机高速输入和视觉验收 |
| M5 事件底座 | Journal、Segment、Blob、Manifest、100 万事件回放 | 高负载采集不影响输入；崩溃恢复无逻辑丢段 |
| M6 AI 垂直切片 | `:brain`、AIDL、OpenAI-compatible、流式、6 个 Skill | Brain 被杀不影响输入；异步结果不覆盖新文本 |
| M7 记忆与快照 | 词汇/纠错/App/风格记忆、索引、快照编译、A/B 回放 | 个性化相对基线出现可重复增益 |
| M8 稳定化 | 机型矩阵、24h 压测、安装包、故障降级、发布检查 | 所有 P0 门禁通过，形成首个内测版本 |

连续两轮真实输入反馈使 M3、M4 优先关闭候选质量、渐进选词和高速触控缺口。事件事实底座顺延到 M5，AI、记忆和稳定化阶段依次顺延；四方向 Skill、编辑事务和 Arctic Glass 三级回退仍保留为后续独立交互迭代。

## 12. 迭代记录：M0 可运行骨架

以下保留 M0 的实施记录；当前代码位于 M4 的 `v0.3.1-m4` 修复预览，仍需完成 GitHub Actions、Android 真机安装、SQLite/剪贴板进程恢复、渐进式候选质量与高速输入性能验收。
...
如果 M2 无法在中端 Android 设备上满足候选延迟与 RSS 预算，项目暂停叠加 AI，优先修复输入内核；如果 M5 影响 IME 稳定性，则 Brain 整体可关闭并继续发布纯输入版本。
...
**下一步：** 先让 `v0.3.1-m4` 的 GitHub Actions 阻断门禁全部通过，再在 Android 10+ 真机完成启用、切换、`hua / shanghua / d / pipei`、候选多页、短促上滑、Emoji 翻页、高速触控、横竖屏与候选延迟 Macrobenchmark。固定预览签名建立前，不把旧 APK 覆盖安装视为已支持能力。
