...
**项目状态：** M1.1 手机键盘布局校正版已构建；等待 Android 真机交互验收
...
## 0. 当前迭代：M1.1 手机键盘布局校正

M1.1 将参考截图中的手机布局固化为可测试契约：第三行严格为 `Shift Z X C V B N M 退格`，第四行严格为 `符 123 ， 空格 。 中/英 回车`，最底部另起独立系统栏，仅在左右两端放输入法选择器和剪贴板，中间完整留白。中文模式默认开启，连续全拼通过 39,002 键的本地词典生成候选。APK 不声明网络权限。

| 本地门禁 | 结果 |
|---|---|
| `core-input` 单元测试 | 13/13 通过 |
| `ime-ui` 布局契约测试 | 3/3 通过 |
...
- **全量归档、有限激活：** 可采集事件逻辑上完整保留，但 IME 只读取固定体积、不可变的用户快照。
- **固定热路径：** 历史从 0 增长到 10 年，逐键延迟不应随档案规模增长。
...
- librime 中文解码底座与 Sense 自定义 Ranker；
...
| 进程 | 负责 | 明确禁止 |
|---|---|---|
| `:ime` | InputMethodService、Canvas UI、librime、Ranker、Journal 入队 | 网络、Provider SDK、历史扫描、数据库查询、JSON 解析 |
...
| 中文内核 | 首轮验证 librime 1.17.x + JNI | 先获得成熟中文能力，再用数据判断是否自研 |
...
| M2 中文引擎 | librime/JNI、全拼、词典、基础 Ranker、用户词 | 候选 p95 达标；连续输入无崩溃和明显分配 |
| M3 手势与视觉 | 四方向 Skill 手势、编辑事务、Arctic Glass 三级回退 | 误触实验通过；模糊开关不影响可读性和帧率 |
| M4 事件底座 | Journal、Segment、Blob、Manifest、100 万事件回放 | 高负载采集不影响输入；崩溃恢复无逻辑丢段 |
| M5 AI 垂直切片 | `:brain`、AIDL、OpenAI-compatible、流式、6 个 Skill | Brain 被杀不影响输入；异步结果不覆盖新文本 |
| M6 记忆与快照 | 词汇/纠错/App/风格记忆、索引、快照编译、A/B 回放 | 个性化相对基线出现可重复增益 |
| M7 稳定化 | 机型矩阵、24h 压测、安装包、故障降级、发布检查 | 所有 P0 门禁通过，形成首个内测版本 |

毛玻璃原型从原设计的最后阶段提前到 M3，因为它涉及 IME Window、OEM、GPU 和节电模式，必须在 UI 架构仍易调整时验证。全量档案则拆成 M4 的可回放事实底座和 M6 的记忆编译，避免首轮同时承担存储、检索和学习三类不确定性。

## 12. 当前开发：M0 可运行骨架

当前代码已形成首个可安装工程预览。接下来先完成真机验收与基准，再进入 M1；目标不是“搭一堆空模块”，而是在真实输入框中得到可启用、可测量的 Sense 输入法。
...
**下一步：** 在 Android 10+ 真机安装 `v0.1.0-m0`，完成启用、切换、真实输入、横竖屏与冷/热启动 Macrobenchmark；通过后关闭 M0，进入 M1 离线输入闭环。
