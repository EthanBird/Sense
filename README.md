...
**项目状态：** M3 句级排序与候选分页工程预览；等待 Android 真机输入与性能验收
...
M3 在 M2 的扩充词典、短码纠错和 SQLite 用户词库之上增加句级排序。构建器从主词典 exact 候选中确定性派生 46,651 个汉字 bigram，形成 559,822 字节的只读 `SBGM` v1 模型；解码器在原有有界 beam 句级组合路径上叠加候选段之间的相邻汉字转移分数，避免对词条内部已经计入 unigram 的关系重复奖励；模型缺失或损坏时退回 M2 排序。候选栏保留左对齐首屏，并为溢出候选提供展开、分页和稳定的全局候选索引。该模型不包含用户输入数据，普通输入仍完全本地运行，APK 不声明网络权限。

| 本地门禁 | 结果 |
|---|---|
| 拼音词库构建器测试 | 3/3 通过 |
| Bigram 构建器测试 | 3/3 通过 |
| `core-input` 单元测试 | 39/39 通过 |
| `ime-ui` 布局契约测试 | 8/8 通过 |
...
本次发布仍是工程预览：当前环境没有连接 Android 设备，因此安装、启用、真实 InputConnection、候选分页触控、横竖屏和帧时间 Macrobenchmark 仍标为待测，Host 基准不能替代真机质量结论。M2 发布 APK 使用临时 debug 证书，M2 到 M3 可能无法覆盖安装；卸载旧版会清除其本地 SQLite 用户词库。成熟 librime 内核仍按后续阶段评估接入。
...
用户对中文长句排序和候选数量的反馈使 M3 优先关闭输入内核的直接可用性缺口。原计划的四方向手势、编辑事务和 Arctic Glass 三级回退尚未在本轮交付，保留为后续独立 UI 里程碑；M4 的可回放事实底座与 M6 的记忆编译边界不变。

## 12. 迭代记录：M0 可运行骨架

以下保留 M0 的实施记录；当前代码已推进到 M3，仍需完成 Android 真机安装、SQLite 重启恢复、句级候选质量与输入性能验收。
...
**下一步：** 在 Android 10+ 真机安装 `v0.3.0-m3`，完成启用、切换、真实中文输入、候选分页、横竖屏与候选延迟 Macrobenchmark；用真实回放扩充正负例后再决定 M4。
