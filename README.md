...
**项目状态：** M7 固定键盘几何与四字词库扩充预览；等待 GitHub Actions 与 Android 真机验收

**最新预览：** `v0.3.4-m7`（`versionCode 10`，prerelease）
...
## 0. 当前迭代：v0.3.4-m7 固定键盘高度与四字词库扩充

`v0.3.4-m7` 继续停留在 `0.3.x`：本轮先修复工具栏与候选切换导致的键盘纵向跳动，再扩充默认中文词库和精确四字简拼排序。Android `versionCode` 从 9 单调增加到 10；发布 tag 固定为 `v0.3.4-m7`，只允许创建 prerelease。

本轮完整保留 M6 的英文组合态、横滑候选、Emoji 惯性、22 类符号、语义候选、文字编辑、高速触控、加速退格和九宫格，并加入：

- 键盘竖屏/横屏总高由 400/300dp 收为 358/258dp，移除删除品牌行后遗留的 42dp 死空间；
- 工具栏与折叠候选栏共用固定 45dp 槽位，composition 和异步候选返回不再改变字母区起点与四排按键行高；
- 导入 30,246 条带拼音的开放成语记录，唯一中文词条增至 166,924，唯一四字词增至 46,597；
- 精确简拼增加字数一致奖励，并为四字词设置更高优先级；
- 固定生产资产回放 `scxt → 上窜下跳`、`ssyw → 蛇鼠一窝`，两者均为首候选。

当前验证状态如下。只有 GitHub Actions 的 `verify` 作业全部通过，工作流才会创建 `v0.3.4-m7` prerelease；未实际执行的 Kotlin、Lint、APK 或真机检查不会写成“已通过”。
...
| Kotlin 正确性 | 独立 Kotlin/JUnit 编译回归：core 78/78、service 22/22、UI 纯逻辑 61/61，共 161/161 本地通过；Android View、Lint 与正式 Gradle 任务以 Actions 结果为准 |
| M7 布局与词库 | 顶部固定 45dp、竖/横屏总高 358/258dp；生产资产首候选 `scxt → 上窜下跳`、`ssyw → 蛇鼠一窝` |
...
| APK 元数据门禁 | `versionCode 10`、`versionName 0.3.4-m7`、`minSdk 29`、`targetSdk 36` |
...
本次发布仍是工程预览：Host 状态机与延迟门禁不能替代真机质量结论。固定发布签名尚未建立，每次 GitHub runner 生成的 debug 证书可能不同，因此即使 `versionCode` 增加，`v0.3.4-m7` 也不保证能覆盖安装旧 APK。签名不一致时卸载旧版会同时清除本地 SQLite 用户词库和剪贴板历史。完整决策见 [`ADR 0009`](docs/adr/0009-v0.3.4-m7-fixed-height-and-idioms.md)，变更与限制见 [`v0.3.4-m7 发布说明`](docs/releases/v0.3.4-m7.md)。
...
| M8 事件底座 | Journal、Segment、Blob、Manifest、100 万事件回放 | 高负载采集不影响输入；崩溃恢复无逻辑丢段 |
| M9 AI 垂直切片 | `:brain`、AIDL、OpenAI-compatible、流式、6 个 Skill | Brain 被杀不影响输入；异步结果不覆盖新文本 |
| M10 记忆与快照 | 词汇/纠错/App/风格记忆、索引、快照编译、A/B 回放 | 个性化相对基线出现可重复增益 |
| M11 稳定化 | 机型矩阵、24h 压测、安装包、故障降级、发布检查 | 所有 P0 门禁通过，形成首个内测版本 |

连续多轮真实输入反馈使 M3–M7 优先关闭候选质量、渐进选词、高速触控、工具面板、中英混输、字符可达性、键盘几何和默认词库缺口。事件事实底座顺延到 M8，AI、记忆和稳定化阶段依次顺延；四方向 Skill、完整事件归档和 Arctic Glass 三级回退仍保留在后续里程碑。

## 12. 迭代记录：M0 可运行骨架

以下保留 M0 的实施记录；当前代码位于 M7 的 `v0.3.4-m7` 工程预览，仍需完成 GitHub Actions、Android 真机安装、固定键盘几何、四字简拼质量、SQLite/剪贴板进程恢复、空 composing 跨宿主兼容、候选与 Emoji 惯性、符号字体和高速输入性能验收。
...
**下一步：** 先让 `v0.3.4-m7` 的 GitHub Actions 阻断门禁全部通过，再在 Android 10+ 真机确认输入前后键位零位移、单行候选，以及 `scxt / ssyw / host / fun / zhongwsrf / ji / oumu / pipei`；随后验收最后 composing 字符退格、候选横滑、Emoji 惯性、分类符号、高速触控、横竖屏与候选延迟 Macrobenchmark。固定预览签名建立前，不把旧 APK 覆盖安装视为已支持能力。
