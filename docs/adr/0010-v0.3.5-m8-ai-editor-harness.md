...
如深度模型普遍超过当前 30 秒总预算，后续可另行评估可选锁定手势，但不能偷偷改变 M8 的取消语义。
...
flowchart LR
    ...
    IPC[ai-runtime\nMessenger + Bundle codec]
    BRAIN[ai-runtime :brain\nService + HTTP/SSE]
...
```text
AiBrainEngine.start(BrainRunSpec, BrainEventSink) -> BrainRunHandle
BrainRunHandle.cancel(reason)
ProviderTransport.open(request, sink) -> ProviderCall
```

Android 跨进程层不把 `InputConnection` 或 Android 类型加入这个模块，而是由
`ai-runtime` 将以下协议值编码为有界 `Bundle`，通过私有 Service 的双向
`Messenger` 传输。回调事件：
...
- 私有 `SenseAiBrainService`，`exported=false`、`process=:brain`；
- Messenger/Bundle request、cancel 与 typed event bridge；
- `AtomicFile` Provider 配置与 Android Keystore API Key；
- `HttpURLConnection`/SSE 字节 Transport；
- 每条 Binder 流式正文不超过 2,048 UTF-16 字符且不拆分代理对。

#### `ai-brain`

纯 Kotlin/JVM 引擎，由 `ai-runtime` 托管于私有 `:brain` 进程：

- Provider-neutral request factory 与 registry；
- 有界 Harness loop；
- SSE/非流式响应解码；
- 超时、单次 repair、同步取消和迟到回调门禁；
- Responses/Chat 与 JSON Schema 归一化；
- 协议解析和第一层校验。
...
3. 自定义 Base URL；
4. 自定义模型名和 API Key；
5. `Responses / Chat Completions`；
6. `JSON_SCHEMA / JSON_OBJECT / PROMPT_ONLY` 结构化输出模式；
7. 流式开关；连接/首事件/流空闲/总超时使用 8 / 8 / 8 / 30 秒的有界默认值；
8. 保存时只做 Base URL、模型、协议、密钥可用性等本地配置验证，不发送能力探测请求。
...
- 保存与本地验证不得把 API Key 或完整 URL query 写入日志；M8 不以探测请求验证 Provider 能力。
- OpenAI Responses 请求必须显式使用 `store=false`；发送给 Provider 的快照剔除仅供
  本地 stale/CAS 使用的 `field_identity`、编辑器代际与单调时钟。
...
5. Brain 作废当前 engine run；
6. 当前 `ProviderCall.cancel()` 关闭活动 `HttpURLConnection`；
7. 关闭 SSE 输入流；
...
协议预览的 CI 和 `offline_verify.sh` 禁止 `android.permission.INTERNET`。M8 集成真实
Provider 后门禁改为：
...
已发布的协议预览冻结了协议与 Fake Harness。其余工作不再拆分，也不创建中间标签；
以下内容作为一次完整的 `v0.3.5-m8` 集成迭代共同提交、共同验收：

- Provider Profile 设置页、AtomicFile 与 Android Keystore；
- `brain-api`、私有 `:brain` Service 和跨进程会话契约；
- `AiHoldGestureSession`、固定高度 AI Surface、流式预览与 dead-man switch；
- 全文/选区/窗口快照、stale guard、API 34 `replaceText` 与兼容事务；
- OpenAI Responses 与 OpenAI-compatible Chat Completions；
- SSE、超时、真实网络取消、严格结构化 Patch 与最多一次格式修复；
- `INTERNET`、私有进程、Forbidden Imports、APK 元数据和既有输入性能门禁。

版本从已发布协议预览的 `versionCode 11` 单调增加一次：

| 集成版本 | versionCode | GitHub 标签 | 发布性质 |
| --- | ---: | --- | --- |
| M8 | 12 | `v0.3.5-m8` | Pre-release |

只有协议、Provider、交互、编辑事务、普通输入回归、Lint、APK 和发布校验全部
通过，才允许通过 ChatGPT GitHub connector 提交并创建 `v0.3.5-m8`。任一门禁
失败都不创建标签或 Release；版本继续保持在 `0.3.x`。
