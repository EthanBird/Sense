...
- `./gradlew :core-input:m4CoreBenchmark` gates production `w → 我` and `ygz → 一个字`, replays the progressive `pipei → 匹pei → 匹配` state transitions, and writes initials/progressive lookup latency to `benchmarks/results/m4-core.json`.
